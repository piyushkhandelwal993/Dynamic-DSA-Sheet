...
test("queue concept detector requires explicit window expiry for deque sliding-window pattern", () => {
  const problem = getProblemById("q-008");
  assert.ok(problem);
  ...
  const detection = detectQueueConcepts(problem, analysis);
  assert.equal(detection.matchedConcepts.includes("sliding-window-queue"), false);
  assert.equal(detection.matchedConcepts.includes("deque-technique"), false);
});
