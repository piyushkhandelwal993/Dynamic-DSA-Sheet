...
function detectProgrammingMathTechniques(facts: CodeFacts, content: string): void {
  ...
  if (
    (/\bfor\s*\(\s*(?:int|long long)\s+\w+\s*=\s*1\s*;[\s\S]{0,80}\w+\s*\*\s*\w+\s*<=\s*\w+/.test(content) || /sqrt\s*\(/.test(content)) &&
  ...
  if (
    ...
    /for\s*\(\s*(?:int|long long)\s+\w+\s*=\s*2\s*;[\s\S]{0,80}\w+\s*\*\s*\w+\s*<=\s*\w+/.test(content) &&
    /%\s*\w+\s*==\s*0/.test(content)
  ) {
  ...
  if (
    /vector\s*<\s*bool\s*>\s*\w+\s*\(/.test(content) &&
    /for\s*\(\s*(?:int|long long)\s+\w+\s*=\s*2\s*;[\s\S]{0,100}\w+\s*\*\s*\w+\s*<=\s*\w+/.test(content) &&
    /for\s*\(\s*(?:int|long long)\s+\w+\s*=\s*\w+\s*\*\s*\w+\s*;[\s\S]{0,80}\+=\s*\w+/.test(content)
  ) {
    addFact(facts, "algorithms", "sieve-precomputation", "high", ["boolean vector with prime multiple marking"]);
  }
  ...
  if (
    hasFactInBuckets(facts, "number-reversal") &&
    /==/.test(content) &&
