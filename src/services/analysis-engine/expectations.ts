...
export const conceptExpectations: Record<string, FactExpectation> = {
  ...
  "dynamic-array-usage": anyOf(allOf(fact("array"), fact("loop")), fact("array")),
  "string-builder-usage": anyOf(fact("loop"), fact("array")),
  "collection-iteration-usage": fact("loop"),
  "hash-map-usage": fact("hash-map"),
  "queue-library-usage": anyOf(fact("queue-like"), fact("queue-operations")),
  "deque-library-usage": anyOf(fact("queue-like"), fact("deque-window")),
  "stack-library-usage": anyOf(fact("stack-like"), fact("stack-operations")),
  "monotonic-deque-usage": fact("deque-window"),
  "priority-queue-usage": fact("priority-queue"),
  "frequency-map-basics": anyOf(fact("hash-map"), fact("frequency-counting")),
  "seen-set-membership": fact("hash-map"),
  "value-to-index-mapping": fact("hash-map"),
  "heap-top-k-usage": anyOf(fact("bounded-priority-queue"), fact("priority-queue")),
  "tokenization-usage": fact("loop"),
  "string-normalization-usage": anyOf(fact("loop"), fact("array")),
  "custom-sorting-helpers": fact("sorting"),
  "pair-collection-usage": anyOf(fact("array"), fact("sorting")),
  "comparator-sorting-usage": fact("sorting"),
  "map-iteration-order-usage": anyOf(fact("hash-map"), fact("sorting")),
  "ordered-map-usage": anyOf(fact("hash-map"), fact("sorting")),
  "heap-of-pairs-usage": anyOf(fact("priority-queue"), fact("bounded-priority-queue")),
  "arraylist-basics": anyOf(allOf(fact("array"), fact("loop")), fact("array")),
  "dynamic-list-insert-delete": anyOf(allOf(fact("array"), fact("loop")), fact("array")),
  "arraylist-iteration-and-conversion": allOf(fact("array"), fact("loop")),
  "nested-list-usage": allOf(fact("array"), fact("loop")),
  "adjacency-list-building": anyOf(fact("graph-adjacency"), allOf(fact("array"), fact("loop"))),
  "min-max-array": fact("min-max-tracking"),
  "matrix-traversal": allOf(fact("array"), fact("loop")),
  "boundary-traversal": allOf(fact("array"), fact("loop")),
  ...
  "opposite-end-pointers": fact("two-pointers"),
  "partition-two-pointers": allOf(fact("two-pointers"), fact("in-place-array-update")),
  "in-place-array-update": fact("in-place-array-update"),
  "sliding-window": fact("sliding-window"),
  "fixed-size-window": fact("sliding-window"),
  "variable-size-window": fact("sliding-window"),
  "window-auxiliary-structure": anyOf(fact("deque-window"), fact("queue-like"), fact("hash-map")),
  "prefix-balance": anyOf(fact("prefix-sum"), fact("hash-map")),
  "prefix-modulo": allOf(fact("prefix-sum"), fact("hash-map")),
  "stock-profit": fact("stock-profit"),
  "string-traversal": fact("loop"),
  "char-classification": fact("loop"),
  "string-builder": anyOf(fact("loop"), fact("array")),
  "palindrome-string": anyOf(fact("two-pointers"), fact("loop")),
  "char-frequency": anyOf(fact("hash-map"), fact("frequency-counting")),
  "anagram-check": anyOf(fact("hash-map"), fact("frequency-counting")),
  "two-string-merge": anyOf(fact("loop"), fact("array")),
  "multiple-string-builders": anyOf(fact("loop"), fact("array")),
  "index-based-distribution": anyOf(fact("loop"), fact("array")),
  "row-simulation": anyOf(fact("loop"), fact("array")),
  "direction-toggle": fact("loop"),
  "word-parsing": fact("loop"),
  "string-cleanup-stack": anyOf(fact("stack-like"), fact("stack-operations")),
  "substring-window-unique": anyOf(fact("sliding-window"), fact("hash-map")),
  "subsequence-dp": anyOf(fact("dp-memoization"), fact("bottom-up-dp"), fact("string-dp")),
  "edit-distance-dp": anyOf(fact("dp-memoization"), fact("bottom-up-dp"), fact("string-dp")),
  ...
  "tree-root-to-leaf-paths": anyOf(fact("recursive-tree-traversal"), fact("tree-serialization"), fact("loop")),
  "tree-value-search": fact("recursive-tree-traversal"),
  "ancestor-max-tracking": anyOf(fact("recursive-tree-traversal"), fact("loop")),
  "root-to-node-path": anyOf(fact("recursive-tree-traversal"), fact("loop")),
  "lca-using-paths": anyOf(fact("lowest-common-ancestor"), fact("recursive-tree-traversal")),
  "subtree-target-count": anyOf(fact("recursive-tree-traversal"), fact("loop")),
  "level-processing": anyOf(fact("level-order-tree-traversal"), fact("queue-like")),
  "level-boundaries": anyOf(fact("level-order-tree-traversal"), fact("tree-view")),
  "queue-with-pair": anyOf(fact("level-order-tree-traversal"), fact("queue-like"), fact("array")),
  "virtual-position": anyOf(fact("level-order-tree-traversal"), fact("array")),
  "relative-index": anyOf(fact("level-order-tree-traversal"), fact("loop")),
  "width-calculation": anyOf(fact("level-order-tree-traversal"), fact("loop")),
  "parent-mapping": anyOf(fact("recursive-tree-traversal"), fact("hash-map")),
  "bidirectional-tree": anyOf(fact("recursive-tree-traversal"), fact("level-order-tree-traversal"), fact("graph-bfs")),
  "wave-expansion-bfs": anyOf(fact("graph-bfs"), fact("level-order-tree-traversal"), fact("loop")),
  "complete-tree-properties": anyOf(fact("tree-height"), fact("recursive-tree-traversal"), fact("loop")),
  "perfect-subtree-counting": anyOf(fact("tree-height"), fact("loop"), fact("recursive-tree-traversal")),
  ...
  "bst-topic-intro": anyOf(fact("tree-node"), fact("bst-search")),
  "bst-topic-search": fact("bst-search"),
  "bst-topic-inorder-sorted": anyOf(fact("recursive-tree-traversal"), fact("bst-search")),
  "bst-topic-validate-range": anyOf(fact("recursive-tree-traversal"), fact("bst-search")),
  "bst-topic-insert": fact("bst-mutation"),
  ...
  "variable-jump-cost-dp": anyOf(fact("dp-state-transition"), fact("bottom-up-dp"), fact("dp-memoization")),
  "jump-reachability-dp": anyOf(fact("dp-state-transition"), fact("bottom-up-dp"), fact("dp-memoization")),
  "cooldown-state-dp": anyOf(fact("dp-state-transition"), fact("bottom-up-dp"), fact("dp-memoization")),
  "probability-dp": anyOf(fact("grid-dp"), fact("bottom-up-dp"), fact("dp-memoization")),
  "transaction-state-dp": anyOf(fact("dp-state-transition"), fact("bottom-up-dp"), fact("dp-memoization")),
  "resource-budget-dp": anyOf(fact("knapsack-dp"), fact("dp-state-transition"), fact("bottom-up-dp")),
  "segment-merge-dp": anyOf(fact("interval-dp"), fact("dp-state-transition"), fact("bottom-up-dp")),
